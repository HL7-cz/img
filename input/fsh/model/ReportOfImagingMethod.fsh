Logical: LogEnReportOfImagingMethodCz
...
* partOfBody 0..* Base "A.2.4.5 - Body part" """The body part that is the subject of an examination or procedure. At least one of the Body Part Code or Description elements must be present."""
...
* procedure 0..* Base "A.2.4.6 - Performance (procedure)" """Description of the procedure expressed by code and/or text description. (E.g. used MR sequences, image projection...)"""
...
* procedure.anatomicalFocus 0..1 Base "A.2.4.6.7 - Anatomical focus of performance" """The anatomical site where the procedure is targeted. Must contain at least one of the elements, either a code and/or a human-readable description of the site on/in the body."""
...
* procedure.complication 0..* Base "A.2.4.6.8 - Complications during the procedure" """Any complications that occurred during or immediately after the procedure. These are usually tracked separately from the notes, which usually describe the procedure itself, not 'post-procedure' problems."""
...
* procedure.comment 0..* Base "A.2.4.6.9 - Comment on the performance (procedure)" """Notes and comments on the performance (procedure)."""
...
* procedure.usedDevice 0..* Base "A.2.4.6.10 - Used medical device" """A medical device used during a procedure."""
...
* medication 0..* Base "A.2.4.7 - Medication" """Any substance administered in connection with a given procedure (typically a contrast agent, but it can also be another type of drug, e.g. a drug to reduce intestinal motility)."""
...
* examinationReport 1..1 Base "A.2.4.9 - Examination report" """Section Reports from imaging methods containing narrative result and optionally formalized result using coding systems"""
* examinationReport.narrativeText 1..1 string "A.2.4.9.1 - Narrative text" """Summary textual (narrative) description of the results of imaging methods"""
* examinationReport.formalizedText 0..* Base "A.2.4.9.2 - Formalized text" """Formalized structured examination result"""
* examinationReport.formalizedText.note 0..* string "A.2.4.9.2.1 - Note" """A note about the result. It should have an author and a timestamp."""
* examinationReport.formalizedText.observationGroup 0..* string "A.2.4.9.2.2 - Observation (measurement) group" """This is a grouping mechanism. An observation can be part of a group of observations (for example, a test panel) that are ordered, performed, or reported together."""
...
* examinationReport.formalizedText.specimen 0..1 string "A.2.4.9.2.4 - Specimen" """Information about the specimen used in the observation."""
* examinationReport.formalizedText.referenceRanges 0..1 string "A.2.4.9.2.5 - Reference ranges" """Information on the reference range relevant to the target population."""
* examinationReport.formalizedText.reasonForAbsence 0..1 string "A.2.4.9.2.6 - The reason for the absence of a value" """The reason why the expected value of the observation is missing."""
* examinationReport.formalizedText.observationValue 0..1 string "A.2.4.9.2.7 - Observation value (measurement)" """The resulting value of the observation. There can be different types of values, either numeric, textual, coded, or raw data may be present."""
* examinationReport.formalizedText.interpretation 0..* string "A.2.4.9.2.8 - Interpretation of the result" """Interpretation of the result of observation (measurement)."""
* examinationReport.formalizedText.observationMethod 0..1 string "A.2.4.9.2.9 - Observation (measurement) method" """Information about the method, principle, or mechanism of observation."""
* examinationReport.formalizedText.derivedFrom 0..* string "A.2.4.9.2.10 - Derived from" """The source from which this observation value is derived. For example, fetal measurements based on an ultrasound image."""
* examinationReport.formalizedText.observedBodyPart 0..1 string "A.2.4.9.2.11 - Observed body part" """Specification of the observed body part. """
* examinationReport.formalizedText.observationStatus 0..1 string "A.2.4.9.2.12 - Observation status" """The status of the observation as final, preliminary, or changed."""
* examinationReport.formalizedText.observationCode 1..1 CodeableConcept "A.2.4.9.2.13 - Observation code" """Coded information about the type of observation.
- Preferred system(s): SNOMED CT"""
* examinationReport.formalizedText.observationCategory 0..* string "A.2.4.9.2.14 - Observation category" """Classification of the general category of observation, e.g. imaging, laboratory, vital signs, etc."""
* examinationReport.formalizedText.observationEquipment 0..* string "A.2.4.9.2.15 - Observation equipment used" """The device used to make an observation and obtain a result."""
* examinationReport.formalizedText.dateTimeObservation 0..1 dateTime "A.2.4.9.2.16 - Observation date and time" """Date and time or period of observation."""
* examinationReport.dateTimeResult  0..1 dateTime "A.2.4.9.3 - Result release date and time" """The date and time the observation was made available, usually after verification."""
* examinationReport.author 0..1 HumanName "A.2.4.9.4 - Result author" """The healthcare professional and/or healthcare provider organization where or by whom the observation was authorized."""
* conclusion 0..1 Base "A.2.4.10 - Conclusion" """The conclusion summarizes the most important findings and/or diagnoses that were made based on the imaging findings (and possibly other clinical information). This section may include a coded assessment, such as BI-RADS categories or equivalent."""
...
* textPartsOfDocument 0..1 Base "A.2.4.12 - Text parts of the document" """Summary text (narrative) parts of the document."""
